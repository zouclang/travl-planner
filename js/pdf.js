...
const TripPDF = (() => {
    ...
    async function exportPDF(tripData) {
        ...
        // 显示导出中提示
        const toast = _createToast('📄 正在生成 PDF，请稍候…');

        try {
            const container = document.createElement('div');
            container.style.cssText = 'position:absolute;left:-9999px;top:0;width:800px;font-family:system-ui,-apple-system,sans-serif;color:#1a1a1a;font-size:13px;line-height:1.6;';
            ...
            _updateToast(toast, '✅ PDF 已保存！');
        } catch (err) {
            console.error('PDF export error:', err);
            _updateToast(toast, '❌ 导出失败：' + err.message);
        }
    }
